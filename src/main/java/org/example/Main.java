...
import org.example.asymmetric.AsymmetricCLI;
import org.example.hash.HashUtil;
import org.example.symmetric.AESService;
import org.example.symmetric.DESService;
import org.example.symmetric.TripleDESUtil;
...
public class Main {
    ...
    public static void symmetricMenu() {
        ...
        System.out.println("3. 3DES (Triple DES)");
        ...
        try {
            switch (algo) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    handleTripleDES(action, text, keyOption);
                    break;
                default:
    ...
    // ================= 3DES =================
    public static void handleTripleDES(int action, String text, int keyOption) throws Exception {
        String key;

        if (keyOption == 1) {
            System.out.print(" Enter Base64 Key (168-bit / 24 bytes): ");
            key = sc.nextLine();
        } else {
            key = TripleDESUtil.generateKey();
            System.out.println(" Key (Base64): " + key);
        }

        if (action == 1) {
            String cipher = TripleDESUtil.encrypt(text, key);
            System.out.println(" Encrypted: " + cipher);
        } else {
            String plain = TripleDESUtil.decrypt(text, key);
            System.out.println("Decrypted: " + plain);
        }
    }
    ...
    public static void hashMenu() {
        ...
        switch (algo) {
            case 1:
                result = HashUtil.hashMD5(text);
                System.out.println(" MD5: " + result);
                break;
            case 2:
                result = HashUtil.hashSHA256(text);
